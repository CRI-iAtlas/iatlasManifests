samples_to_tags_anders <- function() {
  ...
  clinical_df <- synapse_csv_id_to_tbl(syn, "syn65887903")

  # patient_tags <- synapse_csv_id_to_tbl(syn, "syn65900260") %>%
  #   dplyr::select(
  #     "patient_name" = "name",
  #     "patient_id" = "id")
  ...
  tags <- #keep this code, and add a "add_row"statement with synapse id in case new tags were added for this dataset
    ...
    dplyr::add_row(
      synapse_csv_id_to_tbl(syn, "syn65913705") #Anders specific
    ) %>%
    dplyr::select(
      "tag_name" = "name",
      "tag_id" = "id"
  ...
  immune_subtypes <- synapse_csv_id_to_tbl(syn, "syn65888207") %>% #update with synapse id for results for immune subtype classifier
    dplyr::mutate(
  ...
  tide_result <- synapse_tsv_id_to_tbl(syn, "syn65888206") %>% #update with synapse id for results for TIDE scores
    dplyr::mutate(
