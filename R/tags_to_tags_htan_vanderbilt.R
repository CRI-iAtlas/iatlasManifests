tags_to_tags_htan_vanderbilt <- function() {
  ...
  #getting the new group tags from HTAN Vanderbilt
  tags <-
    synapse_csv_id_to_tbl(syn, "syn53697423") %>%
    dplyr::select(
      "tag_name" = "name",
      "tag_id" = "id"
    )
  ...
  tags_to_tags <- synapse_csv_id_to_tbl(syn, "syn60211210") %>%
    dplyr::inner_join(tags, by = dplyr::join_by("tag" == "tag_name")) %>%
    dplyr::inner_join(parent_tags, by = dplyr::join_by("parent_group" == "parent_tag")) %>%
